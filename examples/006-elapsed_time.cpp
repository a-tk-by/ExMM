...
struct Controller006 final : public ControllerBase<HookTypes::Read, Registers>
{
    ...
    void HookRead(Registers* data, size_t offset) override
    {
        ...
        .Case<int>(&Registers::Time, [this](volatile int& time)
        {
            const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - whenStarted);
...
EXMM_DEMO(ElapsedTimeRegisterAccess)
{
    output << "Register Time reads time elapsed since controller start" << std::endl;
