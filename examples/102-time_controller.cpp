...
struct Registers
{
    ...
    {
        uint32_t Start : 1;             // Controller starts counting when 1 is written in this bit. Controller restarts every time when 1 is written here. This bit always reads as 0.
...
struct Controller009 final : public ControllerBase<HookTypes::ReadWrite, Registers>
{
    ...
    void HookRead(Registers* data, size_t offset) override
    {
        ...
        .Case<uint32_t>(&Registers::ControlWord, [this](volatile uint32_t& value)
        {
            value = shadowControl;
            std::cout << value << " ControlWord";
        })
        .Case<uint32_t>(&Registers::StatusWord, [this](volatile uint32_t& value)
        {
            value = shadowStatus;
        ...
        .Case<uint32_t>(&Registers::CurrentTimeLo, [this](volatile uint32_t& value)
        {
            value = latchedCurrentTimeLo;
            std::cout << value << " CurrentTimeLo";
        })
        .CaseArray<uint32_t, 3>(&Registers::NotUsed, [](size_t, volatile uint32_t& value)
        {
            value = 0;
    ...
    void HookWrite(Registers* data, size_t offset) override
    {
        ...
        .InsideArray<Registers::TimerData, 8>(&Registers::Timers, [](std::size_t index, FieldHelper<Registers::TimerData>& next)
        {
            ...
            next
            .Case<uint32_t>(&Registers::TimerData::TriggerTimeHi, [](volatile uint32_t& value)
            {
                std::cout << " hi " << value;  
            })
...
private:
    ...
    {
        std::cout << std::endl << "Control register changed from " << shadowControl << " to " << value << " and then to ";
    ...
    {
        const auto duration = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - whenStarted).count();
    ...
    void TimerThread()
    {
        ...
        switch (timerThreadControl)
        {
            ...
            default:
                ...
                const auto status = ((shadowStatus & Registers::StatusBits_t::TimerInterruptMask) >> Registers::StatusBits_t::TimerInterruptShift);
                const auto control = ((shadowControl & Registers::ControlBits_t::TimerInterruptMask) >> Registers::ControlBits_t::TimerInterruptShift);

                std::cout << std::hex << std::setw(8) << "[S:C] " << shadowStatus <<  " " << shadowControl << std::endl;
                std::cout << std::hex << std::setw(8) << "[s:c] " << status << " " << control << " & " << (status & control) << std::endl;
...
EXMM_DEMO(TimeController)
{
    ...
    {
        ExMM::Run([&interruptsTriggered, registers, &output]()
        {
            output << "Interrupt triggered" << std::endl;
    ...
    ExMM::Run([&values, &registers, &output]()
    {
        output << "Initializing timers" << std::endl;
        ...
        output << "Waiting for results" << std::endl;

        std::this_thread::sleep_for(std::chrono::seconds{15});

        output << "Checking" << std::endl;
